...
global gUlt1 := 2 ;deekin ult check
global gUlt2 := 0 ;havilar ult check
	...
	F9::
	{
		if(gIngame > 0) {
			...
			;if(gCurrentPatron = "NP") {
				...
				totRunTime := TimeSpanAdd(gTotal_RunTime, dtCurrentRunTime)
				FileAppend,%CurrentDateTime%`t%nMax_Level%`t%totRunTime%`t%gTotal_Bosses%`t%gTotal_RunCount%`t%gLevel_Number%`t%gCurrentPatron%`n, MadWizard.txt
			;}
		}
		Reload
		return
...
;ToolTips
{
	UpdateToolTip() {
		...
		sToolTip := sToolTip "`nTarget Level: " (gHR_Enabled ?  nHR_CurrentLimit : nMax_Level)
	...
	ShowStatsTip() {
		...
		if (gShowStatTip) {
			...
			dtNow := A_Now
			dtCurrentRunTime := DateTimeDiff(dtLoopStartTime, dtNow)
			totTimeRun := TimeSpanAdd(gTotal_RunTime, dtCurrentRunTime)
			ToolTip, % "Total Time: " totTimeRun "`nAvg Run: " nAvgRuntime "`nRun Count: " gTotal_RunCount "`nBoss Count: " gTotal_Bosses "`nEst. Gems: " nEstGems "*`n* 7.5 per boss", 25, 350, 3
	...
	ShowToolTip(sText := "") {
		...
		gToolTip := gToolTip ? gToolTip "`n" sText : sText
...
;Init Globals/Settings
{
	...
	Init() {
		...
		gSupportTicket 				:= {}
		gSupportTicket.StartX 		:= supportTicketX - 2
		gSupportTicket.StartY 		:= supportTicketY - 2
		gSupportTicket.EndX 		:= supportTicketX + 2
		gSupportTicket.EndY 		:= supportTicketY + 2
		gSupportTicket.Color_1		:= supportTicketCl
...
{	
	SelectAdventure() {
		...
		return (!StartAdventure(townX, townY)) ? 0 : 1
	}
	...
	StartAdventure(townX, townY) {
		...
		else if (FindPixel(oAdventureStart, foundX, foundY)) {
			oAdventureStart.HasFound := 1
...
;Handle Run Events.. Start/Transition/Reset
{
	RunAdventure() {
		...
		if (FindFirstMob()) {
			...
			;looking for the support ticket
			;if (WaitForFindPixel(gSupportTicket, outX, outY, 5000, 250)) {
				;resetAdventureClick(outX, outY)
			;}
			sleep, 100		
		}
		else {
		...
		while (bContinueRun) {
			...
			if (bRunComplete) {
				...
				;briv work
				;if(gCurrentPatron = "NP" and bContinueRun < 1) {
				;	Sleep, 10
				;	Send, w
				;	Sleep, 10
				;	Send, w
	...
	AdventureWindow_Check(wait_time := 5000) {
		...
		;wait for up to 5 second with 4 checks per second for the Target Pixel to show
		return (WaitForPixel(oAdventureWindowCheck, wait_time)) ? 1 : 0
	}
	...
	DoLevel(nLevel_Number) {	
		...
		if (nLevel_Number = 1) {
			...
			if(gCurrentPatron != "V") {
				...
				num:= 0
				loop, 12 {
					if(GetChampSpecValue(A_Index) > 0) {
						num := num + 1
						if(A_Index = 10) {
							havi := 1
						}
		...
		;if (nWaveNumber = 1 and nLevel_Number < 181) { LevelUp(0) }
		
		;note boss levels will be nWaveNumber = 0
		if (nWaveNumber and(nLevel_Number <= gStopChampLeveling or gStopChampLeveling = -1)) {
		...
		bContinueWave := 1
		while (bContinueWave) {
			IfWinActive, Idle Champions ;weird that you can not move up the {, ahk though
			{
				Send, {Right}
			}
			else {	
			...
			else if (gFamiliarCount < 3 and gEnableMouseSweep = 1 and nWaveNumber) {
				MouseSweep()
			}
			else if (gEnableAutoClick = 1) {
	...
	DoLevel_MW(nLevel_Number) {
		...
		if (nSpecial_Level = 14) { ; and gCurrentPatron != "NP") {
			sleep, 500
			if(gUlt1 > 0) {
				Send, 1
				Sleep, 100
			...
			Loop, 8 {
				num := A_Index + 1
				; (havilar 6 mirt, 6 normal) skip | might have to change the numbers later
				if(gCurrentPatron = "V" or (gCurrentPatron != "V" and num != gUlt2)) {
					Send, {%num%}
					Sleep, 100
				}
			}
	...
	ResetAdventure() {		
        ...
        if (WaitForFindPixel(oPixReset_Skip, outX, outY)) {
			resetAdventureClick(outX, outY)
			bFound := bFound + 1
        }
...
;Roster/Champ Functions (Leveling + Specialization)
{
	GetChampEarlyLeveling(nChampNumber) {
		...
		return GetChampValue(sVal)
	}
	
	GetChampSpecValue(nChampNumber) {
		value := (gFormation != -1) ? Champ%nChampNumber%_SpecialOption%gFormation% : Champ%nChampNumber%_SpecialOptionQ
		return GetChampValue(value)
	}
	
	GetChampValue(value) { ;private
		...
		if (value) {
			if (InStr(value, "|")) {
				...
				else if (gCurrentPatron = "S" and split_vals.Count() > 3) {
					value := split_vals[4]
				}
	...
	DoEarlyLeveling() {
		if(true) { ;new way to get speed up slightly faster
			DoEarlyLvling(6) ;shandie to build up the dash
			if(gCurrentPatron != "V") {
				DoEarlyLvling(10) ;havilar
				Sleep, 50
				Send, 1
				Sleep, 5
				Send, 2
			...
			if(gCurrentPatron != "M") {
				DoEarlyLvling(1) ;deekin more gold the better
			}
			DoEarlyLvling(4) ;sentry better before we go
			DoEarlyLvling(7) ;minsc or bv backup
			if(gCurrentPatron = "NP") {
				DoEarlyLvling(5) ;dhadius early for ult
			}
		}
		else {
			MaxChampNumber := (gLevelingMethod = "F") ? ((gAllowF12_Leveling = 1) ? 13 : 12) : 9
			loop, %MaxChampNumber% {
				DoEarlyLvling(A_Index)
			}
		}
	...
	DoEarlyLvling(num) {
		nEarlyLevelVal := GetChampEarlyLeveling(num)
		if (nEarlyLevelVal > 0) {
			LevelUp(num, nEarlyLevelVal)
			if(num = 1 and nEarlyLevelVal > 0) {
			   gUlt1 := -1
			}
		}
	}
	...
	AutoLevelChamps(level_number := 0) {
		if(level_number = 1 or Mod(level_number, 50) = 14) {
			CenterMouse()
			return
		}
		...
		if (level_number) {
			...
			ctr := (is_even_level) ? 2 : 1
			
			while ctr < MaxChampNumber {
				someNum := 12 ;patron DPSer or 12 for Zorbu training
				;if((ctr != someNum and level_number < round(nMax_Level/2)) or ctr = someNum)
				LevelUp(ctr, 6)
	...
	DoSpecial(nSpecialOption) {
		...
		if (WaitForFindPixel_Moving(gSpecialWindowClose, foundX, foundY)) {
			...
			bFound := (FindPixel(gSpecialWindowSearch, foundX, foundY)) ? 1 : 0
		}
		else {
...
;HELPERS
{
	...
	TimeSpanAdd(ts1, ts2) {
		...
		return TimeResult(t1_seconds + t2_seconds)
	}
